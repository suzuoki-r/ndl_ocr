 ...
 * @param {String} es_form_id	EasySPARQLを実行するフォームのid
 */
var EasySPARQL = function(namespaces, limit, es_form_id){
	//PREFIXとして宣言するための名前空間マッピングオブジェクト
	this.ns = namespaces;
	//共通の基本的なSELECT句変数
	this.base_select = "?s ?label (sample(?creator) as ?creator) ?type ";
	//共通の基本的なWHERE句
	...
	this.prop = {
		...
		value: bp + "value",
		within: bp + "within",
		coveredBy: this.base_pfx[1] + ":geoCoveredBy",
		sourceInfo: bp + "sourceInfo",
		provider: this.base_pfx[1] + ":provider"
	};
	...
	this.defs = {
		...
		what: {pfx: ["type"], distinct: true, select_more: null},
		title: {pfx: null, distinct: true, select_more: null},
		keyword: {pfx: ["keyword"], distinct: true, select_more: "?class"},
		text: {pfx: [], distinct: true, select_more: null},
		source: {pfx: ["chname"], distinct: false, select: "?type (count(?s) as ?count) ", modifier: "GROUP BY ?type ORDER BY desc(?count)"}
	};
	...
	//EasySPARQL実行フォームのDOMノードを確認しておく
	this.base_form = es_form_id ? document.getElementById(es_form_id) : document;
};
...
EasySPARQL.prototype = {
	...
	gen_query: function(qvars){
		...
		"\n} " + param.modifier + (param.limit ? "LIMIT " + param.limit : "");
		return query;
	},
	/**
	...
	prepare_one_tp: function(key, val, param){
		...
		else if(def.select) param.select = def.select;
		if(def.modifier){
			param.modifier = def.modifier;
			param.limit = "";
		}
	...
	parse_uri_param: function(qstr){
		...
		qstr.split(/[&;]/).forEach(function(kvs){
			...
			if(kv){
				...
				//他のフォームに影響を与えないよう、EasySPARQL実行フォーム内のみ値をプリセットする
				if((kfld = this.base_form.querySelector("input[name="+kv[1]+"]"))) kfld.value = qvars[kv[1]];
			}
		}, this);
		return qvars;
	},
	...
	where: function(val){
		...
		if((matched = val.match(/^(.{2,3}?)[都府県]?$/))	//{2,3}のみだと"京都府"は"府"も含めてマッチする
			...
			query += this.prop.spatial + " ?sr .\n\t?sr " + this.prop.coveredBy + "/" + this.prop.within + "* <http://geohash.org/" +
			hash.substr(0, 5) + "> ;\n" + "\t\t" + geopat + ".";
	...
	title: function(val){
		...
		if((matched = val.match(/^=(.*)$/))){
			...
			query += "FILTER(bif:contains(?" + qvar + ", '\"" + val + "\"')) .";
		}
	...
	//ソース情報提供者から型別件数を。2019-02-12
	source: function(val){
		return "?s a ?type ; " + 
		this.prop.sourceInfo + "/" + this.prop.provider + " chname:" + val +" .";
	},
	//// execute from Snorql
