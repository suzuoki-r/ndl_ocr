 ...
 * Apache-2 license
 * re-adopted and extended to work with snorql_ldb.js 2018-12-07 by masaka for Japan Search, last modified 2022-08-18
...
function Snorql(){
	...
	this.elts = {};	//map from ids to elements to be set in init_elts();
	 ...
	 * @param {String} desc_query	query after 'describe'
	 * @param {Object} qparam	query parameter
	 * @param {Object} rparam	results parameter
	 * @return {Boolean}	true if the target uri is in this store
	 */
	this.set_desc_param = function(desc_query, qparam, rparam){
		var matched, submatch, is_home_uri,
		desc_uri = desc_query,	//first assume describe against a particular uri
		pfxs = "",
		duri_pat = this.homedef.duri_pat || this.homedef.datauri_pat || null;	//2 properties for historical reason 2022-03-28
		//in case describe as query (not api param)
		if((matched = desc_query.match(/<([^>]+)>/))) desc_uri = matched[1];
		//this match must be tested against original query, not found uri. 2022-08-17
		if(submatch = desc_query.match(/^(.+?)\s*(WHERE)?\s*\x7B/i)){
			...
			qparam.text = "DESCRIBE " + desc_query;
			
			desc_uri = submatch[1].match(/<([^>]+)>/) ? 
			RegExp.$1 : //avoid to use variable as regex pattern in e.g. set_object_tdval(). 2021-05-08
			"?"	//no target uri: must be set first uri in the results set on snoral_ldb.confirm_tgsrc; 2022-08-17
			//if describe against variables, provably nees prefix decls. 2022-08-17
			if(submatch[1].match(/\?\w/)) pfxs = this._getPrefixes();
		}else{
			rparam.resultTitle = "Description of <" +  desc_uri + ">";
			qparam.text = "DESCRIBE <" + desc_uri + ">";
		}
		if(duri_pat && desc_uri.match(duri_pat)){
	...
	this.init = function(pfxtext){
		...
		this.homedef = Snorqldef ? this.init_homedef(uqparam) : {};
		...
		//var uqparam = this.parse_url_qparam(); //moved up
		if(uqparam.endpoint){
	 ...
	 * @param {Object} url query parameters
	 * @return {Object} object for this.home
	 */
	this.init_homedef = function(uqparam){
		var homedef;
		...
		if(typeof(homedef.is_virtuoso) === "undefined") homedef.is_virtuoso = homedef.endpoint_type === "virtuoso";	//for Util.queries.preamble 2021-02-09
		else if(uqparam.dmode === "CBD" && homedef.endpoint_type === "virtuoso") homedef.is_virtuoso = true;	//override "default" (no to use CBD) by url parameter setting. 2022-01-21
	...
	//prepare map in ids definde this.ids to corresponding DOM node
	...
	//parses url query parameters and set uqparam object
	...
	this.prepare_extension = function(qparam, rparam){
		...
		//@@ easysql 2018-12-08
		if(EasySPARQL){
			//added homedef.easysparql_form param 2021-11-14
			if(!(new EasySPARQL(this._namespaces, 200, this.homedef.easysparql_form).doit(this._endpoint, qparam))) return false;
	...
	this.set_qform_text = function(querytext){
		...
		if(Util.queries) querytext = Util.queries.set_ex_comment(querytext, this.uqparam);
	...
	this.submitQuery = function(){
		...
		if(Util.queries) this.elts.qtxt.value = Util.queries.trim_ex_comment(this.elts.qtxt.value);	//delete top comment 2021-11-14
		if(mode === 'browse' || mode === ""){
